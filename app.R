...
# add year column to "all_years_county" data frame
...
# rename columns of "all_years_county"
...
all_years_county$total_population <- as.numeric(all_years_county$total_population)

# calculate percentages of white, black, asian 
all_years_county <- all_years_county %>%
  mutate(
    pct_total_white = as.numeric(all_years_county$total_white)/all_years_county$total_population*100,
    pct_total_black = as.numeric(all_years_county$total_black)/all_years_county$total_population*100,
    pct_total_asian = as.numeric(all_years_county$total_asian)/all_years_county$total_population*100
  )

# split the county name and state name and add state names to the "all_years_county" data set
all_years_county$state_name <- data.frame(matrix(unlist(strsplit(all_years_county$county_name, ", ")),nrow = 32205, byrow = TRUE))$X2
                          ...
                          sidebarLayout(
                            sidebarPanel(
                              selectInput("val", "Select a variable", 
                                          choices = list("Total Population"="total_population","Percent White"="pct_total_white", "Percent Black"="pct_total_black", "Percent Asian"="pct_total_asian"), selected="pct_total_white"
                              ),
...
server <- function(input, output, session) {
  ...
  output$county_map <- renderPlot({
    # set value to the input variable value
    value <- all_years_county[, input$val]
    # set region to the FIPS code
    ...
    # whole map
      ...
      county_choropleth(subset(map_data, map_data$years == input$year))
    }
  ...
  output$table <- DT::renderDataTable({
    ...
    if(input$state !="None"){
      # set value_dt to the value of selected input variable (total population or percentage of selected race) of the state selected and in the year selected
      value_dt <- subset(all_years_county, all_years_county$state_name %like% input$state & all_years_county$year == input$year)[,input$val]
      
      # round percentages to two decimal places
      if(input$val != "total_population" ){
        value_dt <- format(round(value_dt, 2), nsmall = 2)
      }
      
      # county, state names in the state selected
      name_dt <- subset(all_years_county, all_years_county$state_name %like% input$state & all_years_county$year == input$year)$county_name
    ...
    # if a state is not selected, show data for all counties, excluding data for Puerto Rico
    else{
      value_dt <- subset(all_years_county, all_years_county$year == input$year & all_years_county$state_name != "Puerto Rico")[,input$val]
      ...
      # all county, state names
      name_dt <- subset(all_years_county, all_years_county$year == input$year & all_years_county$state_name != "Puerto Rico")$county_name
